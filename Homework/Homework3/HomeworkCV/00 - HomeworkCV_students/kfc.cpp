...
KImageGray KImageGray::BinaryDilate(const int& nType, const int& KernelSize)
{
	KImageGray  igOut = *this;

    // KernelSize가 3 이라면  int(KernelSize/2)*2 = 2 , KernerSize가 5라면 int(KernelSize/2)*2  == 4
    // 시작 좌표 : int(KernelSize/2)
    int half = KernelSize/2;
    if(nType == _FOREGROUND)
    {
        if(KernelSize == 3){
            for(int i=half,ii=Row()-half*2; ii; i++,ii--)
                for(int j=half,jj=Col()-half*2; jj; j++,jj--)
                {
                    if(_ppA[i][j] == 0)
            ...
            for(int i=half,ii=Row()-half*2; ii; i++,ii--)
                for(int j=half,jj=Col()-half*2; jj; j++,jj--)
                {
                    if(_ppA[i][j] == 0)
                        if(_ppA[i-1][j-1] || _ppA[i-1][j] || _ppA[i-1][j+1] || _ppA[i][j-1]
                                || _ppA[i][j+1] || _ppA[i+1][j-1] || _ppA[i+1][j] || _ppA[i+1][j+1]
                                || _ppA[i-2][j-2] || _ppA[i-2][j-1] || _ppA[i-2][j] || _ppA[i-1][j+1] || _ppA[i-1][j+2]
                                || _ppA[i-1][j-2] || _ppA[i-1][j+2] || _ppA[i][j-2] || _ppA[i][j+2]
                                || _ppA[i+1][j-2] || _ppA[i+1][j+2] || _ppA[i+2][j-2] || _ppA[i+2][j-1] || _ppA[i+2][j]
                                || _ppA[i+2][j+1] || _ppA[i+2][j+2])
            ...
            for(int i=half,ii=Row()-half*2; ii; i++,ii--)
                for(int j=half,jj=Col()-half*2; jj; j++,jj--)
                {
                    if(_ppA[i][j])
                        if(_ppA[i-1][j-1] == 0 || _ppA[i-1][j] == 0 || _ppA[i-1][j+1] == 0 || _ppA[i][j-1] == 0
                            || _ppA[i][j+1] == 0 || _ppA[i+1][j-1] == 0 || _ppA[i+1][j] == 0 || _ppA[i+1][j+1] == 0
                            || _ppA[i-2][j-2] == 0 || _ppA[i-2][j-1] == 0 || _ppA[i-2][j] == 0 || _ppA[i-1][j+1] == 0 || _ppA[i-1][j+2] == 0
                            || _ppA[i-1][j-2] == 0 || _ppA[i-1][j+2] == 0 || _ppA[i][j-2] == 0 || _ppA[i][j+2] == 0
                            || _ppA[i+1][j-2] == 0 || _ppA[i+1][j+2] == 0 || _ppA[i+2][j-2] == 0 || _ppA[i+2][j-1] == 0 || _ppA[i+2][j] == 0
                            || _ppA[i+2][j+1] == 0 || _ppA[i+2][j+2] == 0)
...
KImageGray KImageGray::BinaryErode(const int& nType, const int& KernelSize)
{
	KImageGray  igOut = *this;
...
KImageColor KImageGray::Labeling(const int& NeighborType, int& NLabel)
{
    KImageColor icOut(this->Row(),this->Col());
    ...
    KImageWord iwColor(_nRow, _nCol); // 라벨 분류 변수

    unsigned char uCur, uLeft, uUp, uLeftUp;
    unsigned short wLeftColor, wUpColor, wLeftUpColor;
    ...
    if(NeighborType == 4)
    {
        for(int i = 0; i<_nRow; i++)
        {
            for(int j = 0; j<_nCol; j++)
            {
                ...
                if(uCur == 0) continue; // 배경 무시


                //범위를 벗어나는 메모리에 접근 예외처리
                uLeft = (j > 0) ? _ppA[i][j-1] : 0;
                uUp = (i > 0) ? _ppA[i-1][j] : 0;

                if(j > 0) {
                    wLeftColor = iwColor._ppA[i][j-1];
                } else {
                    wLeftColor = 0;
                }

                if(i > 0) {
                    wUpColor = iwColor._ppA[i-1][j];
                } else {
                    wUpColor = 0;
                }
                ...
                if(uUp == 0 && uLeft == 0)
                {
                    ...
                    opBlob->points.push_back(KPOINT(j,i));
                    iwColor._ppA[i][j] = lBlobList.size();
                }

                //case : 위와 같은 라벨로 분류
                else if(uUp && uLeft == 0)
                {
                    iwColor._ppA[i][j] = wUpColor;
                    lBlobList[wUpColor-1]->points.push_back(KPOINT(j,i));
                }

                //case : 아래와 같은 라벨로 분류
                else if(uUp==0 && uLeft)
                {
                    iwColor._ppA[i][j] = wLeftColor;
                    lBlobList[wLeftColor-1]->points.push_back(KPOINT(j,i));
                }

                //case : 왼쪽의 라벨을 오른쪽 라벨로 merge
                else
                {
                    ...
                    else // uUpColor의 label로 uLeftColor를 merge
                    {
                        for(KPOINT& pt : lBlobList[wLeftColor-1]->points) {
                            // iwColor 업데이트
                            iwColor._ppA[pt.nY][pt.nX] = wUpColor;
                        }
                        //lBloblist[wUpColor-1]에 lBlobList[wLeftColor-1]의 정보를 더해주고
                        lBlobList[wUpColor-1]->points.insert(lBlobList[wUpColor-1]->points.end(), lBlobList[wLeftColor-1]->points.begin(), lBlobList[wLeftColor-1]->points.end());

                        //wLeftColor의 label을 제거
                        lBlobList[wLeftColor-1]->valid = false;

                        //새로운 좌표 j,i를 lBlobList[wUpColor-1]에 추가
                        iwColor._ppA[i][j] = wUpColor;
                        lBlobList[wUpColor-1]->points.push_back(KPOINT(j,i));
                    }
        ...
        // 라벨에 따라 색 랜덤으로 배정
        std::vector<KCOLOR32> labelColors;
        KCOLOR32 randomColor;

        int cnt = 0;
        for(KBlob* blob : lBlobList) {
            if(blob->valid) {
                cnt++;
            }
        }
        NLabel = cnt;
        for(int i=0; i<lBlobList.size(); i++)
        {
            randomColor.r = rand() % 256;
            randomColor.g = rand() % 256;
            randomColor.b = rand() % 256;
            labelColors.push_back(randomColor);
        }

        for(int i=0; i<lBlobList.size(); i++)
        {
            if(lBlobList[i] == nullptr || lBlobList[i]->valid == false) continue;
            KCOLOR32 currentColor = labelColors[i];
            for(int j=0; j<lBlobList[i]->points.size(); j++)
            {
                icOut._ppA[lBlobList[i]->points.at(j).nY][lBlobList[i]->points.at(j).nX].r = currentColor.r;
                icOut._ppA[lBlobList[i]->points.at(j).nY][lBlobList[i]->points.at(j).nX].g = currentColor.g;
                icOut._ppA[lBlobList[i]->points.at(j).nY][lBlobList[i]->points.at(j).nX].b = currentColor.b;
            }
        }
        // 픽셀값 0~255의 값을 가지도록 예외처리
        for(int i=0; i<icOut.Row(); i++){
            for(int j=0; j<icOut.Col(); j++)
            {
                if(!(icOut._ppA[i][j].r >= 0 && icOut._ppA[i][j].r <= 255 && icOut._ppA[i][j].g >= 0 && icOut._ppA[i][j].g <= 255 && icOut._ppA[i][j].b >= 0 && icOut._ppA[i][j].b <= 255))
                    icOut._ppA[i][j].r = icOut._ppA[i][j].g = icOut._ppA[i][j].b = 0;
            }
        }

    }
    // 8-Neighbor Type
    else if(NeighborType == 8)
    {
        for(int i=0; i<_nRow; i++){
            for(int j=0; j<_nCol; j++)
            {
                ...
                uLeft = (j > 0) ? _ppA[i][j-1] : 0;
                uUp = (i > 0) ? _ppA[i-1][j] : 0;
                if(i > 0 && j > 0) uLeftUp = _ppA[i-1][j-1];
                else uLeftUp = 0;
                ...
                if(i > 0 && j > 0) wLeftUpColor = iwColor._ppA[i-1][j-1];
                else wLeftUpColor = 0;

                if(uCur){
                    if(uUp == 0 && uLeft == 0 && uLeftUp == 0)
                    {
                        KBlob* opBlob = new KBlob;
                        lBlobList.push_back(opBlob);
                        opBlob->points.push_back(KPOINT(j,i));
                    ...
                    // case : 왼쪽의 라벨로 배정
                    else if(uLeftUp == 0 && uUp == 0 && uLeft && wLeftColor)
                    {
                        iwColor._ppA[i][j] = wLeftColor;
                        lBlobList[wLeftColor-1]->points.push_back(KPOINT(j,i));
                    }
                    // case : 위쪽의 라벨로 배정
                    else if(uLeftUp == 0 && uUp && uLeft == 0 && wUpColor)
                    {
                        iwColor._ppA[i][j] = wUpColor;
                        lBlobList[wUpColor-1]->points.push_back(KPOINT(j,i));
                    }
                    // case : 왼쪽위의 라벨로 배정
                    else if(uLeftUp && uUp == 0 && uLeft == 0 && wLeftUpColor)
                    {
                        iwColor._ppA[i][j] = wLeftUpColor;
                        lBlobList[wLeftUpColor-1]->points.push_back(KPOINT(j,i));
                    }
                    // case : 왼쪽 위, 왼쪽, 위 모두 같은 라벨
                    else if(uLeftUp && uUp && uLeft && wLeftUpColor)
                    {
                        iwColor._ppA[i][j] = wLeftUpColor;
                        lBlobList[wLeftUpColor-1]->points.push_back(KPOINT(j,i));
                    }

                    else if(uLeftUp == 0 && uUp && uLeft)
                    {
                        if((wLeftColor == wUpColor) && wUpColor && wLeftColor)
                        {
                            iwColor._ppA[i][j] = wUpColor;
                            lBlobList[wUpColor-1]->points.push_back(KPOINT(j,i));
                        }
                        else if( (wLeftColor != wUpColor) && wUpColor && wLeftColor)
                        {
                            for(KPOINT& pt : lBlobList[wLeftColor-1]->points) {
                                // iwColor 업데이트
                                iwColor._ppA[pt.nY][pt.nX] = wUpColor;
                ...
                {
                    if(uUp && uLeft && (wUpColor != wLeftColor) && wUpColor && wLeftColor){
                        for(KPOINT& pt : lBlobList[wLeftColor-1]->points) {
                            // iwColor 업데이트
                            iwColor._ppA[pt.nY][pt.nX] = wUpColor;
                        }
    ...
    for(auto blob : lBlobList)
    {
        if(blob) delete blob;  // nullptr check before deletion
    }
