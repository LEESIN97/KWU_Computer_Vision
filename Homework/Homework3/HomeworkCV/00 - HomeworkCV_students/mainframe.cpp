...
//                                    (_q_pFormFocused->ID() == "Eroded Binary Image" ||
//                                    _q_pFormFocused->ID() == "Dilated Binary Image") &&
//                                    _q_pFormFocused->ImageGray().Address());
    }
...
void MainFrame::on_pushDilation_clicked()
{
    ...
    ImageForm* q_pForm = 0;
    for(auto item: _lImageForm)
        if(item->ID() == "Dilated Binary Image" && item->size() == _q_pFormFocused->size())
        {
            q_pForm = item;
            break;
        }
    ...
    {
        q_pForm = new ImageForm(igBin, "Dilated Binary Image", this);
        q_pForm->show();
        _lImageForm.push_back(q_pForm);
    }
...
void MainFrame::on_pushErosion_clicked()
{
    ...
    ImageForm* q_pForm = 0;
    for(auto item: _lImageForm)
        if(item->ID() == "Eroded Binary Image" && item->size() == _q_pFormFocused->size())
        {
            q_pForm = item;
            break;
        }
    ...
    {
        q_pForm = new ImageForm(igBin, "Eroded Binary Image", this);
        q_pForm->show();
        _lImageForm.push_back(q_pForm);
    }
