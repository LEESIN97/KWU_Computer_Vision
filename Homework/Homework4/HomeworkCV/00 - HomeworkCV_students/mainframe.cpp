...
{    
    ...
    {
        ui->pushOtsuThresholding->setEnabled(_q_pFormFocused &&
        ...
        ui->pushClosing->setEnabled(_q_pFormFocused &&
                                    _q_pFormFocused->ID() == "Binary Image" &&
                                    _q_pFormFocused->ImageGray().Address());
        ui->pushBoundaryExtractor->setEnabled(_q_pFormFocused &&
                                              (_q_pFormFocused->ID() == "Binary Image" || _q_pFormFocused->ID() == "Closing Image" || _q_pFormFocused->ID() == "Opening Image") &&
                                              _q_pFormFocused->ImageGray().Address());
    }
...
{
    KImageGray igBin = _q_pFormFocused->ImageGray().BinaryOpening(_WHITE,ui->editKernelSize->text().toInt());
...
{
    KImageGray igBin = _q_pFormFocused->ImageGray().BinaryClosing(_WHITE,ui->editKernelSize->text().toInt());
...
void MainFrame::on_pushOtsuThresholding_clicked()
{
    KImageGray igMain = _q_pFormFocused->ImageGray();
    KImageGray igBin;
    KHisto().Ostu(&igMain, &igBin);
...
void MainFrame::on_pushBoundaryExtractor_clicked()
{
    KImageGray igMain = _q_pFormFocused->ImageGray().BoundaryExtractor(ui->editBoundaryType->text().toInt(), ui->editNeighborType->text().toInt());
    ...
    {
        q_pForm = new ImageForm(igMain, "Boundary Image", this);
        q_pForm->show();
        _lImageForm.push_back(q_pForm);
    }
