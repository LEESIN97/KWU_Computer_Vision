...
KImageGray KImageGray::BinaryOpening(const int& nType, const int& KernelSize)
{
    KImageGray igTemp = *this;
    KImageGray igOut;
    ...
    if(nType == _FOREGROUND)
    {
        if(KernelSize == 3){
            ...
            for(int i=half,ii=Row()-half*2; ii; i++,ii--)
                for(int j=half,jj=Col()-half*2; jj; j++,jj--)
                {
                    if(igTemp._ppA[i][j] == 0)
                        if(igTemp._ppA[i-1][j-1] || igTemp._ppA[i-1][j] || igTemp._ppA[i-1][j+1] || igTemp._ppA[i][j-1]
                            || igTemp._ppA[i][j+1] || igTemp._ppA[i+1][j-1] || igTemp._ppA[i+1][j] || igTemp._ppA[i+1][j+1])
                            igOut._ppA[i][j] = 255;
                }
            ...
            for(int i=half,ii=Row()-half*2; ii; i++,ii--)
                for(int j=half,jj=Col()-half*2; jj; j++,jj--)
                {
                    if(igTemp._ppA[i][j] == 0)
                        if(igTemp._ppA[i-1][j-1] || igTemp._ppA[i-1][j] || igTemp._ppA[i-1][j+1] || igTemp._ppA[i][j-1]
                            || igTemp._ppA[i][j+1] || igTemp._ppA[i+1][j-1] || igTemp._ppA[i+1][j] || igTemp._ppA[i+1][j+1]
                            || igTemp._ppA[i-2][j-2] || igTemp._ppA[i-2][j-1] || igTemp._ppA[i-2][j] || igTemp._ppA[i-1][j+1] || igTemp._ppA[i-1][j+2]
                            || igTemp._ppA[i-1][j-2] || igTemp._ppA[i-1][j+2] || igTemp._ppA[i][j-2] || igTemp._ppA[i][j+2]
                            || igTemp._ppA[i+1][j-2] || igTemp._ppA[i+1][j+2] || igTemp._ppA[i+2][j-2] || igTemp._ppA[i+2][j-1] || igTemp._ppA[i+2][j]
                            || igTemp._ppA[i+2][j+1] || igTemp._ppA[i+2][j+2])
    ...
    else if(nType == _BACKGROUND)
    {
        if(KernelSize == 3){
            ...
            for(int i=half,ii=Row()-half*2; ii; i++,ii--)
                ...
                {
                    if(igTemp._ppA[i][j])
                        if(igTemp._ppA[i-1][j-1] == 0 || igTemp._ppA[i-1][j] == 0 || igTemp._ppA[i-1][j+1] == 0 || igTemp._ppA[i][j-1] == 0
                            || igTemp._ppA[i][j+1] == 0 || igTemp._ppA[i+1][j-1] == 0 || igTemp._ppA[i+1][j] == 0 || igTemp._ppA[i+1][j+1] == 0)
            ...
            for(int i=half,ii=Row()-half*2; ii; i++,ii--)
                for(int j=half,jj=Col()-half*2; jj; j++,jj--)
                {
                    if(igTemp._ppA[i][j])
                        if(igTemp._ppA[i-1][j-1] == 0 || igTemp._ppA[i-1][j] == 0 || igTemp._ppA[i-1][j+1] == 0 || igTemp._ppA[i][j-1] == 0
                            || igTemp._ppA[i][j+1] == 0 || igTemp._ppA[i+1][j-1] == 0 || igTemp._ppA[i+1][j] == 0 || igTemp._ppA[i+1][j+1] == 0
                            || igTemp._ppA[i-2][j-2] == 0 || igTemp._ppA[i-2][j-1] == 0 || igTemp._ppA[i-2][j] == 0 || igTemp._ppA[i-1][j+1] == 0 || igTemp._ppA[i-1][j+2] == 0
                            || igTemp._ppA[i-1][j-2] == 0 || igTemp._ppA[i-1][j+2] == 0 || igTemp._ppA[i][j-2] == 0 || igTemp._ppA[i][j+2] == 0
                            || igTemp._ppA[i+1][j-2] == 0 || igTemp._ppA[i+1][j+2] == 0 || igTemp._ppA[i+2][j-2] == 0 || igTemp._ppA[i+2][j-1] == 0 || igTemp._ppA[i+2][j] == 0
                            || igTemp._ppA[i+2][j+1] == 0 || igTemp._ppA[i+2][j+2] == 0)
...
KImageGray KImageGray::BinaryClosing(const int& nType, const int& KernelSize)
{
    KImageGray  igTemp = *this;
    KImageGray  igOut;
    int half = KernelSize/2;
...
KImageGray KImageGray::BoundaryExtractor(const int& BoundaryType, const int& NeighborType)
{
    int half = 1;
    KImageGray igBin = *this;
    //background
    if(BoundaryType == 2){
        //Mb4
        if(NeighborType == 4)
        {
            ...
            for(unsigned int i=0; i<igBin.Row(); i++)
                for(unsigned int j=0; j<igBin.Col(); j++)
                {
                    if(igBin[i][j] == 0) continue;

                    else if(_ppA[i][j]) igBin[i][j] = 0;
                }
        ...
        if(NeighborType == 4)
        {
            ...
            for(unsigned int i=0; i<_nRow; i++)
                for(unsigned int j=0; j<_nCol; j++)
                {
                    ...
                    else if(igBin._ppA[i][j]) _ppA[i][j] = 0;
                }
        }
        //Mb8
        else if(NeighborType == 8)
