...
{    
    ...
    {
        ui->pushLuminanceColorTransform->setEnabled(_q_pFormFocused &&
...
void MainFrame::on_pushLuminanceColorTransform_clicked()
{
    // 포커스된 컬러이미지의 주소 불러오기
    KImageColor icMain;
    ...
    // ui에서 설정된 a, b, a', b'을 기반으로 컬러이미지를 Contrast transform을 시켜 새로운 이미지에 저장
    KImageColor icOut = icMain.LContrastTransform(ui->spinInternsityInputLower->value(), ui->spinIntensityInputUpper->value(), ui->spinIntensityOutputLower->value(), ui->spinIntersityOutputUpper->value());


    ImageForm* q_pForm = 0;

    // 이미지폼 리스트에서 contrast transform id를 가지는 이미지폼 불러오기
    for(auto item: _lImageForm)
    ...
    // 이미 똑같은 ID를 가지는 이미지폼이 있다면 Update
    if(q_pForm)
