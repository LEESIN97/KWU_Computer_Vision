...
KImageColor KImageColor::LContrastTransform(const int &nA, const int &nB, const int &nAp, const int &nBp)
{
#ifdef _DEBUG
    assert(nB > nA && nB<256 && nA<256);
#endif

    KImageColor icOut(*this);
    double       dXin, dSlope = (double)(nBp-nAp) / (double)(nB-nA);
    ...
    for(int i=0,ic=Row(); ic; i++,ic--)
        for(int j=0,jc=Col(); jc; j++,jc--)
        {
            ...
            icOut._ppA[i][j].r = (unsigned char)(dSlope * (dXin-nA) + nAp);
        }
    ...
    for(int i=0,ic=Row(); ic; i++,ic--)
        for(int j=0,jc=Col(); jc; j++,jc--)
        {
            ...
            icOut._ppA[i][j].g = (unsigned char)(dSlope * (dXin-nA) + nAp);
        }
    ...
    for(int i=0,ic=Row(); ic; i++,ic--)
        for(int j=0,jc=Col(); jc; j++,jc--)
        {
            ...
            icOut._ppA[i][j].b = (unsigned char)(dSlope * (dXin-nA) + nAp);
        }
    return icOut;
}
