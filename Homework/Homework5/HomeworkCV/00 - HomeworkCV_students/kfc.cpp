...
KImageColor& KImageColor::HistoEqualized()
{
    int npHisto[3][256] = { 0 };
    int nSize = this->Size();
    ...
    for(int row=0; row<Row(); row++){
        for(int col=0; col<Col(); col++){
            npHisto[0][_ppA[row][col].r]++;
            npHisto[1][_ppA[row][col].g]++;
            npHisto[2][_ppA[row][col].b]++;
        }
    }
    ...
    for(int k=0; k<3; k++){
        for(int i=1, ic=255; ic; i++, ic--)
            npHisto[k][i] += npHisto[k][i-1];
    }

    //Histogram Equalization
    for(int k=0; k<3; k++){
        for(int i=1, ic=255; ic; i++, ic--)
            npHisto[k][i] = (int)((double)(npHisto[k][i]) / (double)(nSize) * 255.0);
    }

    //transform the original image
    for(int row=0; row<Row(); row++){
        for(int col=0; col<Col(); col++){
            _ppA[row][col].r = (unsigned char)(npHisto[0][_ppA[row][col].r]);
            _ppA[row][col].g = (unsigned char)(npHisto[1][_ppA[row][col].g]);
            _ppA[row][col].b = (unsigned char)(npHisto[2][_ppA[row][col].b]);
        }
    }
...
KImageColor KImageColor::HistogramMatch(const KImageColor &icRef, const KImageColor &icSrc)
{
    ...
    for(int n=0; n<3; n++)
    {
        ...
        //histograming according to intensities
        dpHisto = vpRefC[n].Pointer();

        for(int ic=igpRef[n].Row(),i=0; ic; ic--, i++)
            for(int jc=igpRef[n].Col(),j=0; jc; jc--, j++)
                dpHisto[igpRef[n]._ppA[i][j]] ++;

        //normalizing
        for(int i=0, ic=vpRefC[n].Size(); ic; ic--, i++)
            dpHisto[i] = dpHisto[i] / (double)(igpRef[n].Size());

        //cumulated histogram
        for(int i=1, ic=255; ic; ic--, i++)
            dpHisto[i] += dpHisto[i-1];
        memcpy(vpRefC[n].Address(), dpHisto, sizeof(double) * vpRefC[n].Size());

        //similarily
        dpHisto = vpSrcC[n].Pointer();

        for(int ic=igpSrc[n].Row(),i=0; ic; ic--, i++)
            for(int jc=igpSrc[n].Col(),j=0; jc; jc--, j++)
                dpHisto[igpSrc[n]._ppA[i][j]] ++;

        for(int i=0, ic=vpSrcC[n].Size(); ic; ic--, i++)
            dpHisto[i] = dpHisto[i] / (double)(igpSrc[n].Size());

        for(int i=1, ic=255; ic; ic--, i++)
            dpHisto[i] += dpHisto[i-1];

        memcpy(vpSrcC[n].Address(), dpHisto, sizeof(double) * vpSrcC[n].Size());
        ...
        for(int ic=igpSrc[n].Row(),i=0; ic; ic--,i++)
            for(int jc=igpSrc[n].Col(),j=0; jc; jc--,j++)
                igpMatched[n]._ppA[i][j] = (unsigned char)(npLookup[igpSrc[n]._ppA[i][j]]);
    }
