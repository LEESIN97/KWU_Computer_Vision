...
{    
    ...
    {
        ui->pushHistogramEqualization->setEnabled(_q_pFormFocused &&
                                                  _q_pFormFocused->ID() == "OPEN" &&
                                                  _q_pFormFocused->ImageColor().Address());
    }
...
void MainFrame::on_pushHistogramEqualization_clicked()
{
    ...
    icMain.HistoEqualized();


    ImageForm* q_pForm = 0;

    for(auto item: _lImageForm)
        if(item->ID() == "Histogram-Equalized Image" && item->size() == _q_pFormFocused->size())
        {
            q_pForm = item;
            break;
        }
    ...
    {
        q_pForm = new ImageForm(icMain, "Histogram-Equalized Image", this);
        q_pForm->show();
        _lImageForm.push_back(q_pForm);
    }
...
void MainFrame::on_pushHistogramMatch_clicked()
{
    QFileDialog::Options    q_Options   =  QFileDialog::DontResolveSymlinks  | QFileDialog::DontUseNativeDialog; // | QFileDialog::ShowDirsOnly
    QString                 q_stFile    =  QFileDialog::getOpenFileName(this, tr("Select a Image File"),  "./data", "Image Files(*.ppm)",0, q_Options);
    if(q_stFile.length() == 0)
        return;
    ...
    q_stFile    =  QFileDialog::getOpenFileName(this, tr("Select a Image File"),  "./data", "Image Files(*.ppm)", 0, q_Options);
    if(q_stFile.length() == 0)
        return;
...
//    KImageColor icMatched = hMatch.HistogramMatching(icTarget, icSrc);
    KImageColor icMatched = icMatched.HistogramMatch(icTarget, icSrc);
